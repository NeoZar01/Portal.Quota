...
namespace DoE.Quota.Repositories.Data
{
    using EF;
    using Api;
    ...
    public class RequisitionRepository : Repository<Requisition>, IRequisitionsRepository
    {
        ...
        public ExecutionResult Park(Requisition requisition, string stage, string state)
        {
            ...
            try
            {
                return db.SaveChanges() == 1 ? ExecutionResult.Success : ExecutionResult.Failure;
            }
            catch
            {
        ...
        public async Task<ExecutionResult> ParkAsync(Requisition requisition, string stage, string state)
        {
            ...
            try
            {
                return await db.SaveChangesAsync() == 1 ? ExecutionResult.Success : ExecutionResult.Failure;
            }
            catch
            {
                throw;
